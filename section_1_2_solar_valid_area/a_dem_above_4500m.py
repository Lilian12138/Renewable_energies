## valid area of DEM above 4500m, output as polygon shapefile
...
dem_path     = os.path.join(base_folder , "processing\gisfiles\DEM\chinadem250.tif")
mask_path    = os.path.join(base_folder , "processing\gisfiles\scratch\dem_4500_mask.tif")
polygon_path = os.path.join(base_folder , "processing\gisfiles\scratch\dem_above_4500m.shp")
...
for y in range(0, ysize, block_y):
    ...
    mask = (arr >= THRESHOLD).astype("uint8")  # >=4500 -> 1, others -> 0
...
layer = out_ds.CreateLayer("dem_above_4500m", srs=srs, geom_type=ogr.wkbPolygon)
