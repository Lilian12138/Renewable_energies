...
dem_path     = os.path.join(base_folder, "processing\gisfiles\slope\chinaslope250.tif")
mask_path    = os.path.join(base_folder, "processing\gisfiles\scratch\slope_30_mask.tif")
polygon_path = os.path.join(base_folder, "processing\gisfiles\scratch\limitedAreasShp\slope_above_30.shp")
...
for y in range(0, ysize, block_y):
    ...
    mask = (arr >= THRESHOLD).astype("uint8")  # >=30 -> 1, others -> 0
...
layer = out_ds.CreateLayer("slope_above_30", srs=srs, geom_type=ogr.wkbPolygon)
