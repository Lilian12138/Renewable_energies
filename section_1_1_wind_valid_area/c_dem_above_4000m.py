## valid area of DEM above 4000m, output as polygon shapefile
from pathlib import Path
base_folder  = Path(__file__).resolve().parents[3]

import os
# os.environ["PROJ_LIB"] = r"D:\installs\Miniforge3\envs\Cct\Library\share\proj"
# os.environ["PROJ_DATA"] = r"D:\installs\Miniforge3\envs\Cct\Library\share\proj"

from osgeo import gdal, ogr, osr

gdal.UseExceptions()

dem_path     = os.path.join(base_folder, r"processing\gisfiles\DEM\chinadem250.tif")
mask_path    = os.path.join(base_folder, r"processing\gisfiles\scratch\dem_4500_mask.tif")
polygon_path = os.path.join(base_folder, r"processing\gisfiles\scratch\limitedAreasShp\dem_above_4000m.shp")
...
for y in range(0, ysize, block_y):
    ...
    mask = (arr >= THRESHOLD).astype("uint8")  # >=4000 -> 1, others -> 0
...
layer = out_ds.CreateLayer("dem_above_4000m", srs=srs, geom_type=ogr.wkbPolygon)
