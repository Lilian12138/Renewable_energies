...
dem_path     = os.path.join(base_folder, "processing\gisfiles\windspeed100m\merged.tif")
mask_path    = os.path.join(base_folder, "processing\gisfiles\scratch\windspeed_4p5_mask.tif")
polygon_path = os.path.join(base_folder, "processing\gisfiles\scratch\limitedAreasShp\windspeed_below_4p5.shp")
...
for y in range(0, ysize, block_y):
    ...
    mask = (arr < THRESHOLD).astype("uint8")  # <4.5 -> 1, others -> 0
...
layer = out_ds.CreateLayer("windspeed_below_4p5", srs=srs, geom_type=ogr.wkbPolygon)
