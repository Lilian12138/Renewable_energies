...
wind_installation = os.path.join(base_folder, r"processing\gisfiles\GridValidArea\grid10km_solar_valid_area_statistic.shp")
...
# Step 3: Multiply normalized value by 5 and write to grid_10km as pv_idx_install
def add_field_safe(fc, field_name, field_type):
    existing = [f.name for f in arcpy.ListFields(fc)]
    if field_name not in existing:
        arcpy.management.AddField(fc, field_name, field_type)

add_field_safe(grid_10km, "pv_idx_install", "DOUBLE")
with arcpy.da.UpdateCursor(grid_10km, [GRID_ID_FIELD, "pv_idx_install"], GRID_FILTER) as cur:
    for row in cur:
        val = cap_dict.get(row[0])
        row[1] = (val - min_val) / val_range if val is not None and val_range > 0 else 0.0
        cur.updateRow(row)

print("Done: pv_idx_install")
